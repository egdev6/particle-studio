...
import { projectLinkResponseMatches } from "../scripts/bootstrap.mjs";
import {
  LIMITS,
  authorizationValue,
  buildPlan,
...
const configuration = {
  ...
  templates: { issueForms: ["bug_report"], issueFormLabels: { bug_report: ["type:bug"] }, pullRequest: true, mode: "ensure" },
  project: {
    title: "Particle Studio",
    fields: { Priority: { dataType: "SINGLE_SELECT", options: ["High", "Low"] } },
...
test("runtime validation enforces schema limits and canonical map identities", () => {
  const invalid = structuredClone(configuration);
  invalid.labels = Object.fromEntries(Array.from({ length: LIMITS.labels + 1 }, (_, index) => [`label-${index}`, { color: "D73A4A" }]));
  ...
  const schema = JSON.parse(fs.readFileSync(path.join(skillRoot, "assets", "config.schema.json"), "utf8"));
...
test("existing project fields skip only when their configuration matches", () => {
  ...
  const matching = { name: "Priority", type: "ProjectV2SingleSelectField", options: [{ name: "High" }, { name: "Low" }] };
  const plan = buildPlan(config, { fields: [matching] });
  assert.equal(plan.find((entry) => entry.resource === "project-field" && entry.target === "Priority").action, "skip");
  assert.throws(
    () => buildPlan(config, { fields: [{ ...matching, dataType: "TEXT" }] }),
    /differs from managed configuration/,
  );
  assert.throws(
    () => buildPlan(config, { fields: [{ ...matching, options: [{ name: "Low" }, { name: "High" }] }] }),
    /differs from managed configuration/,
  );
});

test("template destinations reject symbolic links and permit regular in-repository files", () => {
  const temporaryDirectory = fs.mkdtempSync(path.join(skillRoot, "tests", ".template-destination-"));
  try {
    const repository = path.join(temporaryDirectory, "repository");
    const outside = path.join(temporaryDirectory, "outside");
    fs.mkdirSync(repository);
    fs.mkdirSync(outside);
    fs.symlinkSync(path.join(outside, "config.yml"), path.join(repository, "destination-link"));
    assert.throws(() => writeTemplateFile(repository, "destination-link", "unsafe"), /symbolic link/);

    fs.symlinkSync(outside, path.join(repository, "linked-parent"));
    assert.throws(() => writeTemplateFile(repository, "linked-parent/config.yml", "unsafe"), /symbolic link/);

    writeTemplateFile(repository, ".github/ISSUE_TEMPLATE/config.yml", "safe");
    assert.equal(fs.readFileSync(path.join(repository, ".github/ISSUE_TEMPLATE/config.yml"), "utf8"), "safe");
  } finally {
...
test("legacy resource arrays fail clearly instead of silently accepting duplicate identities", () => {
  const invalid = structuredClone(configuration);
  invalid.labels = [{ name: "type:bug", color: "D73A4A" }, { name: "type:bug", color: "FFFFFF" }];
  assert.equal(errorsFor(invalid).includes("$.labels"), true);
  assert.equal(messagesFor(invalid).includes("legacy arrays are unsupported; use an object map keyed by the managed identity"), true);
});
...
test("normalization sorts resource maps before plan generation", () => {
  ...
  reversed.project.views = { Table: { layout: "TABLE" }, Board: { layout: "BOARD" } };
  const normalized = normalizeConfig(reversed);
  assert.deepEqual(normalized.labels.map((label) => label.name), ["priority:high", "type:bug"]);
  assert.deepEqual(normalized.milestones.map((milestone) => milestone.title), ["Foundation", "Zebra"]);
  assert.deepEqual(normalized.project.fields.map((field) => field.name), ["Priority", "Target date"]);
  assert.deepEqual(normalized.project.views.map((view) => view.name), ["Board", "Table"]);
});
...
test("plan creates missing resources and preserves unmanaged existing resources", () => {
  ...
  const plan = buildPlan(config, {
    ...
    project: { id: "PVT_kwDOExample", title: "Particle Studio", number: 1, linked: false },
    fields: [],
    views: [],
    viewsSupported: true,
  });
  assert.deepEqual(plan.find((entry) => entry.resource === "label" && entry.target === "type:bug").action, "skip");
  assert.deepEqual(plan.find((entry) => entry.resource === "label" && entry.target === "priority:high").action, "update");
  assert.deepEqual(plan.find((entry) => entry.resource === "project-link").action, "link");
  assert.deepEqual(plan.find((entry) => entry.resource === "project-field").action, "create");
  assert.deepEqual(plan.find((entry) => entry.resource === "project-view").action, "create");
  assert.equal(plan.some((entry) => entry.target === ".github/ISSUE_TEMPLATE/config.yml"), true);
});

test("plan reports unavailable GraphQL views without emulation", () => {
  const plan = buildPlan(normalizeConfig(configuration), { viewsSupported: false });
...
test("GitHub Projects v2 layouts map to GraphQL layout enums", () => {
  ...
  assert.throws(() => mapProjectV2ViewLayout("ROADMAP"), /Unsupported configured/);
});

test("duplicate configured project view names fail closed", () => {
  assert.throws(
    () => resolveProjectViewByName([{ id: "PVTV_1", name: "Board" }, { id: "PVTV_2", name: "Board" }], "Board"),
    /Duplicate GitHub Projects v2 view name/,
  );
  assert.throws(
    () => buildPlan(normalizeConfig(configuration), { viewsSupported: true, views: [{ id: "PVTV_1", name: "Board" }, { id: "PVTV_2", name: "Board" }] }),
    /Duplicate GitHub Projects v2 view name/,
  );
});
...
test("project view plan preserves unmanaged default views", () => {
  ...
  const configuredBoard = plan.find((entry) => entry.resource === "project-view" && entry.target === "Board");
  assert.equal(configuredBoard.action, "create");
  assert.equal(plan.some((entry) => entry.resource === "project-view" && entry.target === "View 1"), false);
  assert.equal(plan.some((entry) => entry.action === "update" || entry.action === "delete"), false);
});
...
test("createProjectV2View GraphQL arguments bind the exact input", () => {
  ...
  assert.deepEqual(args.slice(4), [
    "-F", "input[projectId]=PVT_kwDOExample",
    "-F", "input[name]=Board",
    "-F", "input[layout]=BOARD_LAYOUT",
    "-F", "input[configuration][visibleFieldIds][]=PVTF_priority",
    "-F", "input[configuration][visibleFieldIds][]=PVTF_target_date",
  ]);
});
...
test("repository binding accepts only exact supported GitHub HTTPS and SSH origins", () => {
  for (const origin of [
    ...
    assert.equal(repositoryBindingMatches("EgDev6/Particle-Studio", origin), true);
  }
  for (const origin of [
  ...
  assert.equal(repositoryBindingMatches("egdev6/particle-studio", "git@github.com:egdev6/other.git"), false);
});

test("duplicate project titles fail closed before identity selection", () => {
  assert.throws(
    () => resolveProjectByTitle([{ id: "PVT_1", number: 1, title: "Particle Studio" }, { id: "PVT_2", number: 2, title: "Particle Studio" }], "Particle Studio"),
    /Duplicate GitHub Projects v2 title/,
  );
  assert.deepEqual(resolveProjectByTitle([{ id: "PVT_1", number: 1, title: "Particle Studio" }], "Particle Studio"), { id: "PVT_1", number: 1, title: "Particle Studio" });
});

test("authorization hash binds the exact canonical plan inputs", () => {
  const inputs = {
    config: normalizeConfig(configuration),
    target: { repoDir: "/work/particle-studio", origin: "git@github.com:egdev6/particle-studio.git", repository: configuration.repository },
    observed: { labels: [], milestones: [], templates: [], project: null, fields: [], views: [], viewsSupported: null, repositoryId: "R_1" },
    plan: [{ resource: "label", target: "type:bug", action: "create", reason: "missing" }],
  };
  ...
  reordered.labels = { "priority:high": configuration.labels["priority:high"], "type:bug": configuration.labels["type:bug"] };
  assert.equal(authorizationValue({ ...inputs, config: normalizeConfig(reordered) }), authorization);
...
test("failure reports always retain the declared machine-readable shape", () => {
  ...
  assert.deepEqual(Object.keys(report).sort(), ["authorization", "completed", "discovered", "failures", "mode", "plan", "schemaVersion", "skipped", "success", "validation"]);
  assert.equal(report.success, false);
  assert.equal(Array.isArray(report.completed) && Array.isArray(report.skipped) && Array.isArray(report.failures), true);
  assert.deepEqual(Object.keys(report.discovered).sort(), ["labels", "milestones", "project", "viewsSupported"]);
});

test("templates enforce review labels, required scoped evidence, and no blank issues", () => {
  const readTemplate = (name) => fs.readFileSync(path.join(skillRoot, "assets", "templates", name), "utf8");
  ...
  const example = JSON.parse(fs.readFileSync(path.join(skillRoot, "assets", "example.config.json"), "utf8"));
  ...
  assert.equal(example.templates.issueFormLabels.bug_report.includes("status:needs-review"), true);
  assert.equal(example.templates.issueFormLabels.feature_request.includes("status:needs-review"), true);
});

test("project link mutation requires the exact configured repository node ID", () => {
  const linked = { data: { linkProjectV2ToRepository: { repository: { id: "R_1" } } } };
  assert.equal(projectLinkResponseMatches(linked, "R_1"), true);
  assert.equal(projectLinkResponseMatches(linked, "R_2"), false);
  assert.equal(projectLinkResponseMatches({ data: { linkProjectV2ToRepository: {} } }, "R_1"), false);
});

test("scope headers are deterministic", () => {
  assert.deepEqual(parseOAuthScopes("HTTP/2 200\r\nx-oauth-scopes: repo, project, workflow\r\n"), ["repo", "project", "workflow"]);
});
